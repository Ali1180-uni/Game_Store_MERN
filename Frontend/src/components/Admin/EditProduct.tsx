...
import { fetchAdminProducts, updateProduct, updateProductStock, deleteProduct, type AdminProduct } from "../../Api/Admin.api";
...
const EditProduct = () => {
  ...
  if (isLoading) return <p className="text-sm text-neutral-500">Loading products...</p>;
                ...
                <td className="px-4 py-3">
                  <input
                    ...
                    onBlur={(e) => {
                      ...
                      const value = Math.trunc(Math.min(MAX_STOCK, Math.max(0, parsed)));
                      e.target.value = String(value);
                      if (value !== p.stock) stockMutation.mutate({ id: p._id, stock: value });
                    }}
      ...
      {deletingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-sm rounded-2xl border border-neutral-800 bg-neutral-900 p-6 text-center">
            <h3 className="text-lg font-semibold text-white">Delete this product?</h3>
            <p className="mt-2 text-sm text-neutral-400">
              This will permanently delete <span className="text-white">{deletingProduct.title}</span>. This can't be undone.
            </p>
...
const EditProductModal = ({ product, onClose }: { product: AdminProduct; onClose: () => void }) => {
  const queryClient = useQueryClient();
  ...
  const {
    ...
    defaultValues: {
      ...
      preOrderReleaseDate: detail?.preOrderReleaseDate ? detail.preOrderReleaseDate.slice(0, 10) : "",
      platform: detail?.platform ?? "",
      brand: detail?.brand ?? "",
    },
  });

  const isPreOrder = useWatch({ name: "preOrder" }) === "true";
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label className="mb-1.5 block text-sm font-medium text-neutral-300">
                Product Image <span className="text-neutral-500">(leave empty to keep current)</span>
              </label>
              <div className="flex items-center gap-3">
                <img src={product.image} alt={product.title} className="h-14 w-14 shrink-0 rounded-lg object-cover" />
                <input
                  type="file"
                  accept="image/*"
                  {...register("image")}
